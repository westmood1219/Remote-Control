...
void CRemoteClientDlg::DealCommand(WORD nCmd, const std::string& strData, LPARAM lParam)
{
    ...
    case 3:
        ...
        MessageBox("open file finished!", "open成功", MB_ICONINFORMATION);
        break;
        // 文件下载
    case 4:
        break;
    case 9:
        MessageBox("delete file finished!", "delete成功", MB_ICONINFORMATION);
        break;
    case 1981:
        MessageBox("测试成功!","连接成功",MB_ICONINFORMATION);
        break;
    default:
        TRACE("未知数句接收!\r\n");
        break;
...
void CRemoteClientDlg::InitUIData()
{
    ...
    UpdateData();
    //m_server_address = 0xC0A85B80;//192.168.91.128
...
void CRemoteClientDlg::UpdateDownloadFile(const std::string& strData, FILE* pFile)
{
    ...
    if (length == 0) {
        length = *(LONGLONG*) strData.c_str();// 尝试从当前数据包头 解析出文件总长度
        if (length == 0) {
        ...
        fwrite( strData.c_str(), 1,  strData.size(), pFile);//1表示每次写入1字节,size表示写入多少次
        index +=  strData.size();
		// 把index拨到length
        //               if (index >= length) {
        //                   fclose( pFile);
...
void CRemoteClientDlg::OnRunFile()
{
	...
	CString strFile = m_List.GetItemText(nSelected, 0);
	strFile = strPath + strFile;
	int ret = CClientController::getInstance()->SendCommandPacket(GetSafeHwnd(), 3, true, (BYTE*)(LPCSTR)strFile );
	if (ret < 0) {
